...
class Contextual_Attention_Module(nn.Module):
    ...
    def forward(self, f, b, mask=None, ksize=3, stride=1,
        ...
        for xi, wi, raw_wi in zip(f_groups, w_groups, raw_w_groups):
            ...
            division = torch.floor_divide(offset, fs[3]).long()
            offset = torch.stack([division, torch.floor_divide(offset, fs[3])-division], dim=-1)
...
def down_sample(x, size=None, scale_factor=None, mode='nearest', device=None):
    ...
    res = F.interpolate(x, scale_factor=scale_factor, mode=mode, recompute_scale_factor=True)  # fix pytorch1.6 warning
    return res
